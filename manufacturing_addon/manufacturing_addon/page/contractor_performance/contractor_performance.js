...
// Route /app/contractor-performance — tabs must NOT use href="#..." (Frappe router).
...
(function () {
	function inject_cp_dashboard_styles() {
		...
		const css = `
			...
			/* Bootstrap 4 has no flex gap — space filters explicitly */
	...
	frappe.pages["contractor-performance"].on_page_load = function (wrapper) {
		try {
			if (window.location.hash || window.location.pathname.indexOf("%23") !== -1) {
				history.replaceState(null, "", "/app/contractor-performance");
			}
		} catch (e) {
			/* ignore */
		}
		...
		function load() {
			...
			frappe
				...
				.then((data) => {
					$meta.text(
						`${__("Period")}: ${data.from_date} — ${data.to_date}. ${__(
							"Source: submitted Cutting, Stitching & Packing reports (child table qty)."
						)}`
					);
